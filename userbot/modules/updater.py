    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await edit_or_reply(
                xx,
                f"{txt}\n"
                "**Kredensial Heroku tidak valid untuk deploy Mikoo-Userbot dyno.**",
            )
            return repo.__del__()
        try:
        ...
        await edit_or_reply(
            xx, "`Mikoo-Userbot Berhasil Di Deploy! Userbot bisa di gunakan kembali.`"
        )
...
async def update(xx, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await edit_or_reply(
        xx, "`Mikoo-Userbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
    )
...
async def upstream(event):
    ...
    off_repo = b64decode(
        "aHR0cHM6Ly9naXRodWIuY29tL2RpdmFydmlhbi9NaWtvby1Vc2VyYm90"
    ).decode("utf-8")
    force_update = False
    try:
        txt = (
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy Mikoo-Userbot Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**✥ Mikoo-Userbot Sudah Versi Terbaru**")
        return repo.__del__()
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}update`\
        \n  •  **Function : **Untuk Melihat Pembaruan Terbaru Mikoo-Userbot.\
        \n\n  •  **Syntax :** `{cmd}update deploy`\
        \n  •  **Function : **Untuk MengUpdate Fitur Terbaru Dari Mikoo-Userbot.\
    "
    }
)
