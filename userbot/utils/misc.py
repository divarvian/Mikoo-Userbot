...
async def animator(media, mainevent, textevent):
    ...
    DivaArvian = await mainevent.client.download_media(media, TEMP_DOWNLOAD_DIRECTORY)
    await textevent.edit("`Converting...`")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {DivaArvian} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an Video.webm"
    )
    os.remove(DivaArvian)
    vid = "Video.webm"
    return vid
